...
class HKex_Search:
    ...
    def file_save(self, participant_name_final, shareholding_final, sharehoding_percent_final, input_stock_code,
        ...
        if os.path.exists(self.path) == True:  # 判断路径存在
            if os.path.exists(self.path + '{}'.format(input_date_data) + '_' + input_stock_code + '.xls'):  # 判断文件存在
                os.system(
                    self.path + '{}'.format(input_date_data) + '_' + input_stock_code + '.xls')  # 自动打开存在路径下已有的excel
                return 0
            if os.path.exists(
                    self.path + '{}'.format(input_date_data) + '_' + input_stock_code + '.xls') == False:  # 误删文件后重新爬取下载
    ...
    def getData(self):
        ...
        dataget.file_save(participant_name_final=participant_name, shareholding_final=shareholding,
