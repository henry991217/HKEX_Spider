...
class HKex_Search:
    ...
    def file_save(self,participant_name_final,shareholding_final,sharehoding_percent_final):#在D盘新建文件夹并将信息保存进excel
        ...
        if os.path.exists(self.path)==True:#判断路径存在
            if os.path.exists(self.path+'{}.xls'.format(dataget.getYesterday())):#判断文件存在
             os.system(self.path+'{}.xls'.format(dataget.getYesterday()))#自动打开存在路径下已有的excel
             return 0
            if os.path.exists(self.path+'{}.xls'.format(dataget.getYesterday()))==False:#误删文件后重新爬取下载
                ...
                workbook.save(self.path+'{}.xls'.format(dataget.getYesterday()))
                os.system(self.path+'{}.xls'.format(dataget.getYesterday()))#打开文件
         ...
         for i in range(len(participant_name_final)):
           ...
           data_sheet.write(i+1,2,label=sharehoding_percent_final[i])
         workbook.save(self.path+'{}.xls'.format(dataget.getYesterday()))
         os.system(self.path+'{}.xls'.format(dataget.getYesterday()))#生成并打开excel



if __name__ == "__main__":
    dataget = HKex_Search()
    dataget.get_data(dataget.getYesterday(), "01865")
